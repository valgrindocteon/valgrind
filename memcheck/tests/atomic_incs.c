...
__attribute__((noinline)) void atomic_add_8bit ( char* p, int n ) 
{
   ...
   /* We rely on the fact that p is 4-aligned. Otherwise 'll' may throw an
      exception that can cause this function to fail. */
#if defined (_MIPSEL)
   unsigned long long block[3]
      = { (unsigned long long)p, (unsigned long long)n, 0x0ULL };
   do {
      __asm__ __volatile__(
         "move $t0, %0"           "\n\t"
         "ld   $t1, 0($t0)"       "\n\t"  // p
...
#elif defined (_MIPSEB)
   unsigned long long block[3]
      = { (unsigned long long)p, (unsigned long long)n << 56, 0x0 };
   do {
      __asm__ __volatile__(
         "move  $t0, %0"          "\n\t"
         "ld    $t1, 0($t0)"      "\n\t"  // p
...
#elif defined (_MIPSEB)
   unsigned long long block[3]
      = { (unsigned long long)p, (unsigned long long)n << 48, 0x0 };
   do {
      __asm__ __volatile__(
         "move  $t0, %0"          "\n\t"
         "ld    $t1, 0($t0)"      "\n\t"  // p
